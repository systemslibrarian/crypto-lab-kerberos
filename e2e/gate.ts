 ...
 *     this file replaces already walked every scenario, which was more than
 *     most gates in this fleet did — but it called `revealEverything` before
 *     each scan, stripping `.hidden` and `[hidden]` off every element and
 *     force-opening every `<details>`. On this lab that fabricates a page no
 *     visitor can reach: the scenario panels are mutually exclusive, and the
 *     attack-outcome and replay panels are `.hidden` until their run produces a
 *     result, so un-hiding them scans empty containers dressed as populated
 *     ones. It also froze `animation` and `transition` to none, which makes the
 *     suite structurally unable to see a transition or theme-swap defect.
 *
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#scenario option').first()).toBeAttached();
 ...
 * plausible offender: it prints hex ticket blobs and a swim-lane diagram whose
 * steps carry `min-width: 420px`.
 */
 ...
 *
 * The scenario `<select>` is the lab's main axis — each value swaps in a
 * different set of panels — so every option is walked. The run / replay / clock
 * / self-check controls are then driven within a scenario, because their result
 * panels start `.hidden` and the replaced gate only ever saw them force-revealed
 * and empty.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  const values = await page.$$eval('#scenario option', (opts) =>
    opts.map((o) => (o as HTMLOptionElement).value)
  );
  expect(values.length, 'the scenario select must offer options').toBeGreaterThan(0);

  for (const value of values) {
    await page.selectOption('#scenario', value);
    await expect(page.locator('#flow')).toBeVisible();
    await expect(page.locator('#scenario-blurb')).not.toBeEmpty();
    await scan(page, `${theme} / scenario: ${value}`);

    // Run this scenario, so its outcome panel renders for real rather than
    // being force-revealed empty.
    const run = page.locator('#run');
    if (await run.isEnabled()) {
      await run.click();
      await expect(page.locator('#flow')).not.toBeEmpty();
      await scan(page, `${theme} / scenario: ${value} / run`);
    }
  }
}
