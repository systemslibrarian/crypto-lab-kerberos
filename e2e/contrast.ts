 ...
 *    files the node under `incomplete`. The swim-lane arrows are painted with a
 *    `linear-gradient` and most surface tints here are `color-mix(...)` washes
 *    over the panel, so their text composites onto whatever is behind rather
 *    than onto a flat colour axe can read.
 ...
 *    `opacity: .85`, and `.swim-step` starts at `opacity: 0` until its reveal
 *    animation runs — the case that turned out to be a real defect here.
 *
 ...
 *    `.swim` is an `overflow-x: auto` scroller around a swim-lane whose steps
 *    carry `min-width: 420px`, so at 380px much of the diagram is clipped away.
